// `gbounce posture` — per-bouncer posture surface (CLI shim).
//
...
func newPostureCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Report gbounce's posture: is it running, what mode + profile,
and is the operator's HTTP_PROXY / HTTPS_PROXY pointing at gbounce.
...
uncertainty + misconfig — if HTTP_PROXY points at loopback but
gbounce isn't running, the output reports "MISCONFIGURED" rather
than silently claiming intercept.
...
shape (ibounce / kbounce / dbounce / gbounce).`,
		...
		RunE: func(c *cobra.Command, _ []string) error {
			...
			w := c.OutOrStdout()
			fmt.Fprintln(w, "== gbounce posture ==")
			status := "STOPPED"
			if block.Running {
				status = "RUNNING"
			}
