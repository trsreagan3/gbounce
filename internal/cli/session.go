// `gbounce session` subcommand group — #285 session recording / playback.
//
...
// gbounce home. Operators override via `--dir`.
func defaultSessionsDir() string {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return filepath.Join(".gbounce", "sessions")
	}
	return filepath.Join(home, ".gbounce", "sessions")
}
...
// The grammar matches the Python ibounce parser + kbouncer + dbounce Go
// parsers so the cross-product UX is identical per
...
the proxy runs with --record-sessions-dir. The proxy reads agent
identity from the inbound X-Agent-Session-Id + X-Agent-Name headers;
requests without a session id are NOT routed to a session file. The
files are portable + replayable via the cross-product
'iam-jit session replay <FILE>' command.
...
func newSessionListCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&dirPath, "dir", "",
		"Recordings dir. Defaults to ~/.gbounce/sessions/.")
