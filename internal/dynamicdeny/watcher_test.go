// watcher_test.go — #324d watcher regression suite.
//
// Covers:
//   - file-creation event triggers reload + emits ReasonFileCreated
//   - file-modification event triggers reload + emits ReasonFileModified
...
// rule id. Reused across the watcher tests so each test asserts a
// distinct payload-id-changes -> rule-id-changes flow.
...
func TestWatcher_DetectsFileCreation(t *testing.T) {
	...
	// File does NOT exist at NewWatcher time.
	er := &emitRecorder{}
	...
	// Capture stderr so the test doesn't pollute test output on errors.
	...
	if err := os.WriteFile(p, []byte(validYAMLPayload(validRuleID, "*.openai.com")), 0o600); err != nil {
	...
	// Reason could be ReasonFileCreated (the typical case) or
	// ReasonFileModified (if the watch attached during the write, the
	// underlying fsevents may surface it as a modify). Both are valid.
...
func TestWatcher_DetectsFileModification(t *testing.T) {
	...
	// Modify with a new rule id.
	if err := os.WriteFile(p, []byte(validYAMLPayload(validRuleID2, "*.anthropic.com")), 0o600); err != nil {
...
func TestWatcher_DebouncesRapidWrites(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		body := validYAMLPayload(validRuleID2, "*.anthropic.com")
	...
	// Allow up to 2 emits (one for the create-shaped first event, one
	// for the final settle). Anything > 2 means we failed to debounce.
	if len(snap) > 2 {
...
func TestWatcher_NoPathStartIsNoOp(t *testing.T) {
	// An empty path → watcher behaves as a no-op (Snapshot empty;
	// Start returns nil; Stop completes immediately).
