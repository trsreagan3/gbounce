...
// GET / on the gbounce mgmt port alongside /healthz + /audit/events
// (#272).
//
...
// Bounce-suite products (ibounce / kbounce / dbounce) ship the same
...
// mutate gbounce state. Per [[security-team-positioning-safety-not-
...
// bouncerNameGbounce is the product name baked into the served HTML.
const bouncerNameGbounce = "gbounce"
...
func auditEventsUIHandler(requireBearer string) http.HandlerFunc {
	body := renderAuditEventsUI(bouncerNameGbounce)
